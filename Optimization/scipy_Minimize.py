...
class Minimize:
    ...
    def Objective_Function(self, AllDecision):
        ...
        CostValue = self.CostF(x, y)

        ApprValue = self.ValueF(x, y)
    ...
    def Minimize_Main(self):
        x0, bnds = self.Generate_DecisionVar()
        WeighCons = {'type': 'ineq', 'fun': self.Weight_Cons}
        PackAllCons = {'type': 'eq', 'fun': self.Pack_Cons}
        VehAllCons = {'type': 'eq', 'fun': self.Veh_Cons}
