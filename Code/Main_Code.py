...
from Code.ADP import ADP_Algorithm
...
def Main_Func(Stop_Number, Graph, T_System, T_Event, Vehicle_Number, Vehicle_Set, Package_Set):
    ...
    for System_T in range(T_System):
        ...
        # 依据某一随机分布，在各集中站点生成订单
        OrderID_New, UnRellocated_Order, Transport_Order = Generate_Order_Step(System_T, OrderID_Last, Package_Set, UnRellocated_Order, Transport_Order, Stop_Number, T_System)
        ...
        if(len(Arrival_List) == 0):
            ...
            UnRellocated_Order, Transport_Order = Find_Package_Vehicle(System_T, Arrival_List, UnRellocated_Order, Transport_Order, Records, Vehicle_Set, Package_Set)
...
# 依据当前系统时间生成各站点订单
@jit(nopython=True)
def Generate_Order_Step(System_T, OrderID_Last, Package_Set, UnRellocated_Order, Transport_Order, Stop_Number, T_System):
    ...
    for i in range(Stop_Number):
        if(Stop_Basic[i]>=0.8):
            ...
            for j in range(Order_Num):
                ...
                Transport_Order.append(OrderID_Last)
    return OrderID_Last, UnRellocated_Order, Transport_Order
...
# 找到每辆已到站点的车搭载的包裹
@jit(nopython=True)
def Find_Package_Vehicle(System_T, Arrival_List, UnRellocated_Order, Transport_Order, Records, Vehicle_Set, Package_Set):
    # 需要找到车辆i搭载的全部包裹j
    for i in Arrival_List:
        # 取出第i辆车的行数据
        Records_I = Records[[i]]
        ...
        for j in OrderList_I:
            ...
            # 包裹已到终点
                ...
                Cur_State = 2
                Package_Set[str(j)].Update_Package_BeforeD(Cur_Origins, Cur_State)
                Transport_Order.remove(j)

            # 配送记录发生变化
            Records[i][j] = 0

    return UnRellocated_Order, Transport_Order
